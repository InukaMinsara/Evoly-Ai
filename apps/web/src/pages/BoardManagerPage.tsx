    ...
    <div className="flex-1 flex flex-col h-full bg-surface overflow-hidden">
      <div className="min-h-16 px-4 sm:px-6 py-3 sm:py-0 border-b border-surface-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-surface-card/50 flex-shrink-0">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <CircuitBoard className="w-5 h-5 text-evoly-500 flex-shrink-0" />
          <h1 className="text-base sm:text-lg font-semibold text-slate-200">Board Manager</h1>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full bg-surface border border-surface-border rounded-lg pl-9 pr-4 py-1.5 sm:py-2 text-xs sm:text-sm text-slate-200 focus:outline-none focus:border-evoly-500"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
        ...
        {/* Installed */}
        <section>
          <h2 className="text-xs sm:text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3 sm:mb-4">Installed Boards</h2>
          {installed.length === 0 ? (
            <div className="text-xs sm:text-sm text-slate-500 italic px-2 sm:px-4">No boards installed yet.</div>
          ) : (
              ...
              {KNOWN_BOARDS.filter(b => installed.includes(b.fqbn)).map(board => (
                <div key={board.id} className="bg-surface-card border border-surface-border rounded-xl p-4 flex flex-col gap-3 group">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-semibold text-slate-200 text-sm sm:text-base">{board.name}</h3>
                  ...
                  <div className="mt-auto pt-3 border-t border-surface-border/50 flex justify-between items-center">
                    ...
                    <button
                      ...
                      className="text-xs text-red-400 hover:text-red-300 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex items-center gap-1"
                    >
