...
import { Cpu, ChevronDown, Wifi, WifiOff, Loader2 } from 'lucide-react';
...
export function TopBar({ title, currentModel, aiStatus, onModelChange }: TopBarProps) {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-5 border-b border-surface-border bg-surface-card/50 backdrop-blur-sm flex-shrink-0">
      {/* Title */}
        ...
        <h1 className="text-sm font-semibold text-slate-200 truncate max-w-xs">
          {title ?? (
            <span className="flex items-center gap-2 text-slate-400 font-normal">
              <Cpu className="w-4 h-4 text-evoly-500" />
              EVOLY AI Assistant
            </span>
          )}
        </h1>
        ...
        <div className="relative">
          ...
          {modelOpen && (
            <>
              ...
              />
              {/* Dropdown */}
              <div className="absolute right-0 top-full mt-1 z-50 w-72 max-h-[70vh] overflow-y-auto bg-surface-card border border-surface-border rounded-xl shadow-xl animate-fade-in custom-scrollbar">
